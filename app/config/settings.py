...
class Settings:
    ...
    # Memory v2 配置
    MEMORY_SUMMARY_TTL: int = _env_int("MEMORY_SUMMARY_TTL", 2592000)       # 摘要 TTL（30天）
    MEMORY_SHARED_TTL: int = _env_int("MEMORY_SHARED_TTL", 86400)           # 共享记忆 TTL（24小时）
    MEMORY_SUMMARY_TRIGGER_INTERVAL: int = _env_int("MEMORY_SUMMARY_TRIGGER_INTERVAL", 10)  # 摘要触发间隔（轮次）
    CONTEXT_MAX_TOKENS: int = _env_int("CONTEXT_MAX_TOKENS", 8000)           # 上下文窗口最大 token 数
