...
async def generate_answer_stream(
    ...
    """使用 PromptBuilder + ContextManager 构造 messages"""
    context_text = []
    ...
    messages = PromptBuilder.build_messages(
        ...
        recent_turns=5 if memory_context else 0,
        include_summary=bool(memory_context),
        include_preferences=bool(memory_context),
        include_history=bool(memory_context),
    )

    cm = get_context_manager()
    messages = cm.fit_messages(messages, rag_context=context_str, quant_context=quant_text)
...
async def rag_quant_pipeline(
    ...
    # 5. LLM 生成（使用 PromptBuilder + ContextManager）
    if streaming:
        response_stream = await generate_answer_stream(query, docs, quant_result, memory_context=memory_context)
        return response_stream
    else:
        answer = await generate_answer(query, docs, quant_result, memory_context=memory_context)
